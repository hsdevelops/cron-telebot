...
async def restrict_to_admins(update, db_service):
...
async def check_rights(update, context, db_service, must_be_admin=False):
...
async def restrict_to_user(update, db_service):
