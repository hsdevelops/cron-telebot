...
async def send_channels_only_error_message(update: Update, chat_type: str):
    reply = channels_only_error_message.replace("__chat_type__", chat_type)
    await update.message.reply_text(reply)


async def send_user_unauthorized_error_message(update: Update, bot_ic: str):
    reply = user_unauthorized_error_message.replace("__bot_ic__", bot_ic)
    await update.message.reply_text(reply)


async def send_wrong_restriction_message(update: Update, bot_ic: str):
    reply = wrong_restrction_error_message.replace("__bot_ic__", bot_ic)
    await update.message.reply_text(reply)


async def send_timezone_nochange_error_message(update: Update):
    await update.message.reply_text(timezone_nochange_error_message)


async def send_invalid_crontab_message(update: Update):
    await update.message.reply_text(
...
async def send_invalid_new_job_message(update: Update):
    await update.message.reply_text(invalid_new_job_message)


async def send_quiz_unavailable_message(update: Update):
    await update.message.reply_text(
...
async def send_no_photos_to_delete_error_message(update: Update):
    await update.message.reply_text(no_photos_to_delete_error_message)


async def send_attribute_change_error_message(update: Update):
    await update.message.reply_text(attribute_change_error_message)


async def send_private_only_error_message(update: Update):
    await update.message.reply_text(private_only_error_message % BOT_NAME)


async def send_missing_chats_error_message(update: Update):
    await update.message.reply_text(missing_chats_error_message % BOT_NAME)


async def send_missing_bot_in_group_message(update: Update):
    await update.message.reply_text(
