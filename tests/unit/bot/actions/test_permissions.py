...
def test_check_rights_creator(
    ...
    simple_update.message.from_user.id = 2
    res = check_rights(simple_update, simple_context, mongo_service)
    assert res is False
...
def test_check_rights_admin(
    ...
    simple_context.bot.get_chat_member = mock.Mock(return_value=m)
    res = check_rights(simple_update, simple_context, mongo_service)
    assert res is exp
