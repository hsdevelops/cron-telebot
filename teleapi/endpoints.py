...
def send_media_group(chat_id: int, photo_id: str, content: str, user_bot_token: str, message_thread_id: int) -> requests.Response:
...
def send_single_photo(chat_id: int, photo_id: str, content: str, user_bot_token: str, message_thread_id: int) -> requests.Response:
    query = {
        "chat_id": chat_id,
        "photo": photo_id,
        "caption": content,
...
def send_single_photo_local(
    new_token: Optional[str], chat_id: int, content: str = "", photo: Optional[str] = None, remote_photo_id: Optional[str] = None, prev_token: Optional[str] = None
) -> requests.Response:
...
def send_poll(chat_id: int, content: str, user_bot_token: str, message_thread_id: int) -> requests.Response:
...
def send_text(chat_id: int, content: str, user_bot_token: str, message_thread_id: int) -> requests.Response:
    query = {
        "chat_id": chat_id,
        "text": content,
        "parse_mode": "html",
...
def delete_message(chat_id: int, previous_message_id: str, user_bot_token: Optional[str] = None) -> Any:
...
def download_photo(files: Dict[str, Any], photo_id: str, bot_token: Optional[str] = TELEGRAM_BOT_TOKEN) -> Dict[str, Any]:
...
def transfer_photo_between_bots(db_service: mongo.MongoService, new_token: Optional[str], prev_token: Optional[str], chat_id: int, entry: COLLECTION_TYPE) -> Tuple[requests.Response, Optional[str]]:
    ...
    if resp.status_code == 200:
        ...
        delete_message(chat_id, str(
            resp.json()["result"]["message_id"]), new_token)
